...
def Book(request,pk):
    if pk:
        ...
        if request.method == "POST":
            ...
            print(uni_id)
            room_count = Rooms.objects.filter(hotel_id=pk).filter(room_type=room_type).filter(room_fac=room_fac).count()
            if(room_count):
                ...
                return render(request,'hotel_booking/sorry_no_room.html',{'hotel':hotel,'room_type':room_type,'room_fac':room_fac})
        else:
